module.exports = {
  ...
  getValidMoves: function(turn, board, lastMove) {
    ...
    //removing the nonjump (invalid)
    for(var i = 0; i<moves.length; i++) {
      if(moves[i].jumpX != -1) {
        moves = deleteNonJumps(moves);
      }
    }
    //cancelling if continueJump is false
    if(continueJump) {
      moves = deleteOtherPiecesMoves(moves, lastMove.piece);
    }
    return moves;
  },
  //get valid positions when jumping
...
//get the properties of the moves
...
// when jumping get position of pieces 
